...
(function () {
    ...
    var containerNode = doc.head || doc.getElementsByTagName('head')[0] || doc.body || doc.documentElement || currentScript.parentNode;
    ...
    function _loadScript(url, callback, id) {
        ...
        script.async = true;
        script.defer = true;
        lastAppendScript = url;
        containerNode.appendChild(script);
        lastAppendScript = undefined;
     ...
     * @private
     */
    function _each(list, callback, isReverse) {
        var i;
        var j;
    ...
    function _getInteractiveScript() {
        if (lastAppendScript) {
            return lastAppendScript;
        } else if (doc.currentScript) {
            return doc.currentScript;
        } else {
            //IE6-10 得到当前正在执行的script标签
            var scripts = doc.scripts || containerNode.getElementsByTagName('script');
            var findScript;

            _each(scripts, function (index, script) {
            ...
            try {
                ...
                stack = e.stack ||
                (win.opera && ((e + "").match(/of linked script \S+/g) || []).join(" "));
            }
